...
const TextFieldsNode = ({ data, id }) => {
  ...
  // Suggestions generated by the autofill system for a given set of input rows "base".
  ...
  // Query the autofill system for more suggestions when the suggestions aren't 
  // already loading and either:
  // (1) there are no more suggestions left to show, or
  // (2) the existing rows that the suggestions were based on have changed.
  useEffect(() => {
    if (isSuggestionsLoading) return;
    const rows = Object.values(textfieldsValues);
    if (suggestedRows.suggestions.length === 0 || !match(suggestedRows.base, rows)) {
      console.log(`base has changed from \n${suggestedRows.base} \nto\n ${rows}`)
      setIsSuggestionsLoading(true);
      autofill(rows, SUGGESTIONS_TO_PRELOAD).then((res) => {
        setSuggestedRows({
          base: rows,
          suggestions: res,
        });
  ...
  }, [isSuggestionsLoading, suggestedRows, textfieldsValues]);
  ...
  function handleKeyDown(event, index) {
    if (event.key === 'Tab' && textfieldsValues[index] === '' && !isSuggestionsLoading) {
      ...
      handleTextFieldChange(index, suggestedRows.suggestions[0]);
      // remove the suggestion from the list
      setSuggestedRows({
        base: suggestedRows.base,
        suggestions: suggestedRows.suggestions.slice(1),
      });
    }
  };
  ...
  function placeholder() {
    ...
    return suggestedRows.suggestions[0];
  }

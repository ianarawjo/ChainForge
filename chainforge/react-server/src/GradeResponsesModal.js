  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" withBorder style={{backgroundColor: checked ? '#f2f7fc' : '#fff'}}>
      <UnstyledButton onClick={() => setChecked(!checked)} onKeyUp={(e) => e.preventDefault()} className="checkcard">
        ...
        <Switch size='lg' color='gray' onLabel='Code' offLabel='LLM' pos='absolute' right='8px' bottom='10px' 
                checked={codeChecked}
                onChange={(e) => setCodeChecked(e.currentTarget.checked)}
                thumbIcon={
                  codeChecked ? (
                    <IconCode size="0.8rem" color={theme.colors.teal[theme.fn.primaryShade()]} stroke={3} />
                  ) : (
                    <IconRobot size="0.8rem" color={theme.colors.blue[theme.fn.primaryShade()]} stroke={3} />
                  )
                }
        />
...
export const GradeResponsesWindow = forwardRef((props, ref) => {
  ...
  const responseText = useMemo(() => ((shownResponse && shownResponse.responses?.length > 0) ? shownResponse.responses[0] : ""));
  const prompt = useMemo(() => (shownResponse?.prompt ?? ""), [shownResponse]);
  const varsDivs = useMemo(() => {
    const combined_vars_metavars = shownResponse ? {...shownResponse.vars, ...transformDict(shownResponse.metavars, (key) => !key.startsWith('LLM_'))} : {};
    return Object.entries(combined_vars_metavars).map(([varname, val]) => 
      <div className="grade-resp-var-container">
        <span key={varname} className="response-var-name">{varname}&nbsp;=&nbsp;</span><span className="response-var-value">{val}</span>
      </div>
    );
  }, [shownResponse]);
  ...
  return (
    ...
    <Box>
      <Flex justify='center'>
        {
          (shownResponseIdx in grades) 
            ? (grades[shownResponseIdx]) 
                ? <HeaderText>
                    You chose&nbsp;<IconThumbUp color='green' style={{marginBottom: '-3px'}} />!
                  </HeaderText>
                : <HeaderText>
                    You chose&nbsp;<IconThumbDown color='red' style={{marginBottom: '-6px'}} />!
                  </HeaderText>
            : <HeaderText>
      ...
      <Flex justify='center' align='center' mb='sm'>
        ...
        <div className="response-box" style={{ backgroundColor: "#eee", width: "80%", maxHeight: '300px', overflowY: 'scroll', borderColor: 'black', borderStyle: 'solid' }}>
          <div className="response-item-llm-name-wrapper">
            <div className="small-response" style={{fontSize:"11pt", padding: '12pt'}}>
              {responseText}
            </div>
          </div>
        </div>
      ...
      <Flex justify='center' mb='xl' gap='lg'>
        <div style={{backgroundColor: '#fff', padding: '12px', width: '31%', borderRadius: '12px', borderWidth: '1px', borderStyle: 'solid'}}>
          Vars
          <hr />
        ...
        <div style={{backgroundColor: '#fff', padding: '12px', width: '41%', borderRadius: '2px'}}>
          Prompt
          <hr />
          <div className='monofont' style={{maxHeight: '300px', overflowY: 'scroll', fontSize: '10pt', lineHeight: '1.2'}}>
            {prompt}
          </div>
        </div>
      </Flex>
      ...
      <Flex justify="center" gap='50px' mb='xl'>
        <Button color='red' 
          ...
          <IconThumbDown />&nbsp;Bad!
          <>{isRedExploding && <ConfettiExplosion zIndex={1000} colors={['#f00']} force={0.1} height={300} width={200} particleCount={5} duration={2200} onComplete={() => setIsRedExploding(false)} style={{position: 'absolute', left: '50%', top: '20%'}} />}</>
        </Button>
        <Button color='green' 
                variant='filled' 
                onClick={() => {
      ...
      <Flex justify='left' align='center' gap='md'>
        ...
        <Text color='#aaa' size='sm'>Generating candidate implementations...</Text>

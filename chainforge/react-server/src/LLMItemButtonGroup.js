...
export default function LLMItemButtonGroup( {onClickTrash, onClickSettings, ringProgress, hideTrashIcon} ) {
    return (<div>
        <Group position="right" style={{float: 'right', height:'20px'}}>
            {ringProgress !== undefined ? 
                ((ringProgress.success > 0 || ringProgress.error > 0) ?
                    (<RingProgress size={20} width='16px' thickness={3} sections={[
                        { value: ringProgress.success, color: ringProgress.success < 99 ? 'blue' : 'green' },

...
const togetherGroups = () => {
  ...
  togetherModels.forEach((model) => {
    ...
    if (!(group in groups)) {
      ...
      (groups[group].items as LLMSpec[]).push(spec);
    }
  });
  return groupNames.map((group) => groups[group]);
};
...
function flattenLLMGroups(groups: LLMGroup[]): LLMSpec[] {
  return groups.flatMap((group) => flattenLLMGroup(group));
}

function flattenLLMProviders(providers: (LLMSpec | LLMGroup)[]): LLMSpec[] {
  return providers.flatMap((item) =>
    "group" in item && "items" in item ? flattenLLMGroups([item]) : item,
  );
}

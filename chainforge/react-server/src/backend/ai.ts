...
export async function autofill(input: Row[], n: number): Promise<Row[]> {
  ...
  // TODO: Check if there's a single template variable across inputs.
  // If so, all template variables generated by the LLM should be renamed to that variable name
  // (since the LLM cannot be trusted to stick to the original name)
